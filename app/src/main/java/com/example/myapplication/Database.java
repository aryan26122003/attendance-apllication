...
public class Database {
    ...
    public void addClass(Class c) {
        try {
            ...
            String create1 = "CREATE TABLE IF NOT EXISTS `" + c.getID() + "_Sessions` (`ID` integer, `Subject` text, `Date` text);";
            statement.execute(create1);
            String create2 = "CREATE TABLE IF NOT EXISTS `" + c.getID() + "_Students`(`ID` integer, `FirstName` text,`LastName` text,`Email` text, `Tel` text);";
    ...
    public Student getStudent(int classID, int studentID){
        ...
        try {
            String select = "SELECT `ID`, `FirstName`, `Email`, `Tel`, FROM `"
    ...
    public void addStudent(int classID, Student s){
        try{
            String insert = "INSERT INTO `"+classID+" - Students` (`ID`, `FirstName`,`LastName`,`Email`,`Tel`) VALUES (`"+s.getID()+"`,`"+s.getFirstName()+"` , `"+s.getLastName()+"` ,`"+s.getEmail()+"`,`"+s.getTel()+"`); ";
            statement.execute(insert);
        }catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void updateStudent(int classID, Student s){
        try{
            String update = "UPDATE `"+classID+"` - Students` SET `FirstName` = `"+s.getFirstName()+"`,`LastName` =`"+s.getLastName()+"` ,`Email` = `"+s.getEmail()+"`,`Tel`=`"+s.getTel()+"` WHERE `ID`=`"+s.getID()+"`;";
